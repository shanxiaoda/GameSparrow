...
public class TSProperties : MonoBehaviour
{
    ...
    public string codeTemplate = "Assets/src/tsproj/UITemplate.txt";
    public string extendsClass = "JsBehaviour";
    public string importPath = @"import JsBehaviour from ""Game/JsBehaviour.mjs"";";
    public bool customCodeTemplate = false;
    public string customCodeTemplateContent = null;

    public int Int_MaskField = 0;
    public int Int_IntPopup = 0;
    public Component ct;

    public int testint = 4;

    [SerializeField]
    public TestObj tt = new TestObj();
    public float[] ff;
    ...
    ** 这里面的组件都不导出
    **/
    private Type[] typeBlackList = new Type[] { typeof(CanvasRenderer) };
    /**
    * 默认名字的都不导出
    **/
    private System.String[] NoExportName = { "Image", "Text (TMP)" };
    ...
    public void updateCustom()
    {
        ...
        {
            Transform t1 = t[i];
            if (t1 == this.transform)
            {
                continue;
            }
            if (this.NoExportName.Contains(t1.name))
            {
                continue;
            }
            list.Add(this.newObject(t1.name, t1.gameObject, j++));
            Component[] aa = t1.GetComponents<Component>();
            for (int k = 0; k < aa.Length; k++)
            {
                Component c = aa[k];
                if (this.typeBlackList.Contains(c.GetType()))
                {
                    continue;
                }
                list.Add(this.newObject(t1.name + "_" + c.GetType().Name, c, j++));
            }
        }
        this.CustomObjectPairs = list.ToArray();
    }
    ...
    [SerializeField]
    private Object[] CustomObjectPairs;
    [SerializeField]
    ...
    private Object newObject(string key, UnityEngine.Object o, int index)
    {
        var obj = new Object();
        obj.value = o;
        obj.key = key;
    ...
    public bool isBlack(Type type)
    {
        for (int i = 0; i < this.typeBlackList.Length; i++)
        {
            if (this.typeBlackList[i] == type)
            {
                return true;
            }
        }
...
namespace UnityEditor.GUI
{
    ...
    [CustomEditor(typeof(CSMonoBehaviour))]
    ...
    public class TSProperties_CustomEditor : Editor
    {
        ...
        void DisplayMenu()
        {
            ...
            if (true)
            {
                ...
                {
                    string content = null;
                    if (_instance.customCodeTemplate)
                    {
                        content = _instance.customCodeTemplateContent;
                    }
                    else
                    {
                        string p = Path.Combine(_instance.codeTemplate.Split("/"));
                        content = File.ReadAllText(p);
                    }

                    string[] scriptPath = _instance.jsScript.Split("/");
                    var list = new List<string>(scriptPath);
                    list.RemoveAt(list.Count - 1);
                    string basePath = Path.Combine("Assets", "src", "tsproj", "ts", "UI");
                    list.Insert(0, basePath);

                    string className = scriptPath[scriptPath.Length - 1];
                    className = "UI_" + Path.GetFileNameWithoutExtension(className);

                    string uiPath = Path.Combine(list.ToArray());

                    if (Directory.Exists(uiPath) == false)
                    {
                        Directory.CreateDirectory(uiPath);
                    }

                    uiPath = Path.Combine(uiPath, className + ".mts");

                    // string uiPath = Path.Combine("Assets", "src", "tsproj", "ts", "UI", className + ".mts");
                    content = content.Replace("{className}", className);
                    var code = DisplayUtility.GenCode(_instance.GenPairs(), "public", false);
                    content = content.Replace("{attribute}", code);
                    content = content.Replace("{import}", _instance.importPath);
                    File.WriteAllText(uiPath, content);

                    Debug.Log("生成UI类:" + uiPath);

                    // for (int i = 0; i < _instance.Pairs.Length; i++)
                    // {
                    //     TSProperties.ResultPair rp = _instance.Pairs[i];
                    //     new TSProperties.ResultPair();
                    //     // rp.key;
                    //     // rp.value;
                    // }
                    // _instance.addNum();
                ...
                _instance.jsScript = EditorGUILayout.TextField("JS Script Name:", _instance.jsScript);
                _instance.codeTemplate = EditorGUILayout.TextField("Code Template:", _instance.codeTemplate);
                _instance.extendsClass = EditorGUILayout.TextField("Extends Class:", _instance.extendsClass);
                _instance.importPath = EditorGUILayout.TextField("Import Path:", _instance.importPath);

                if (_instance.customCodeTemplateContent == null)
                {
                    string pp = "Assets/src/tsproj/UITemplate.txt";
                    _instance.customCodeTemplateContent = File.ReadAllText(Path.Combine(pp.Split("/")));
                }

                _instance.customCodeTemplate = EditorGUILayout.BeginToggleGroup("Custom Code Template:", _instance.customCodeTemplate);

                _instance.customCodeTemplateContent = EditorGUILayout.TextArea(_instance.customCodeTemplateContent, GUILayout.Height(100));

                EditorGUILayout.EndToggleGroup();

                EditorGUILayout.EndVertical();

                _instance.Int_IntPopup = EditorGUILayout.IntPopup("aaaaa", _instance.Int_IntPopup, new string[] { "aaa", "bbbb" }, new int[] { 1, 3 }, new GUILayoutOption[] { GUILayout.Width(400) });
                // Debug.Log(_instance.Int_IntPopup);

                _instance.Int_MaskField = EditorGUILayout.MaskField("bbbbb", _instance.Int_MaskField, new string[] { "aaa", "bbbb", "ccc", "dddd" });

                // Debug.Log("aaaaaaaaaaaaaaaaa");

                // int temp = 1 << 1;

                // Debug.Log(_instance.Int_MaskField << 1);
                // Debug.Log(_instance.Int_MaskField << 2);
                // Debug.Log(_instance.Int_MaskField << 3);
                // Debug.Log(_instance.Int_MaskField << 4);

                // Debug.Log(_instance.Int_MaskField);
                this.testse(4, _instance.Int_MaskField);

                EditorGUILayout.Popup(0, new string[] { "aaa", "bbbb" }, GUILayout.MaxWidth(300));
                ...
                // EditorGUILayout.Toggle("", false, "PaneOptions");
                // EditorGUILayout.Toggle("", false, UnityEngine.GUI.skin.customStyles[1].name);

                // var so = new SerializedObject(_instance);
                // var ff = so.FindProperty("a");
                // EditorGUILayout.PropertyField(ff, true);
                // so.ApplyModifiedProperties();

                // _instance.tt;
                //  _instance.tt;
                var dda = this.serializedObject.FindProperty("ff");
                
                EditorGUILayout.PropertyField(dda, true);
            }
        }
